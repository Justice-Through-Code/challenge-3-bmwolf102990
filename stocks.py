...
def stock_purchases():
    ...
    if stock_name == "Amazon":
        price = f"${amazon}"
        calculation = int(investment_amount / amazon)
    elif stock_name == "Apple":
        price = f"${apple}"
        calculation = int(investment_amount / apple)
    elif stock_name == "Facebook":
        price = f"${fb}"
        calculation = int(investment_amount / fb)
    elif stock_name == "Google":
        price = f"${google}"
        calculation = int(investment_amount / google)
    elif stock_name == "Microsoft":
        price = f"${msft}"
        calculation = int(investment_amount / msft)
    else:
        price = ""
        calculation = 0
    ...
    print(f"{client_name} has ${investment_amount} to invest and can buy {calculation} shares of {stock_name} at the current price of {price}.")
